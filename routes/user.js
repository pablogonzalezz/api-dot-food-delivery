...
router.get('/get_user_phone/:login', async function(req, res, next) {
    
    if (req.params.login) {
        try {
            await getConnection().connect( async (err, client, done) => {
                await client.query(
                    `SELECT phone FROM users WHERE login='${req.params.login}'`
                ).then(results => {
                    if(err) {
                        console.error('error: ', err);
                    }
                    done();  
                    res.status(200).json(results.rows[0].phone);
                })
            });
        } catch (error) {
            console.error('erro ao fazer requisição: ', error);
